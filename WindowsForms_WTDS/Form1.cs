...
namespace WindowsForms_WTDS
{
    ...
    public partial class Form1 : Form
    {
        ...
        private static ModbusRtu masterRtuAup;
        private static ModbusRtu masterRtuAdown;
        private static ModbusRtu masterRtuAleft;
        private static ModbusRtu masterRtuAright;
        private static ModbusRtu masterRtuBup;
        private static ModbusRtu masterRtuBdown;
        private static ModbusRtu masterRtuBleft;
        private static ModbusRtu masterRtuBright;
        ...
        /// </summary>
        public void ModbusRtuMasterWriteSingleRegister()
        {
            masterRtuAup = new ModbusRtu();
            try
            {
                masterRtuAup.SerialPortInni(sp =>
                    {
                        sp.PortName = "COM1";
                        sp.BaudRate = 9600;
                        sp.DataBits = 8;
            ...
            masterRtuAdown = new ModbusRtu();
            try
            {
                masterRtuAdown.SerialPortInni(sp =>
                {
                    sp.PortName = "COM2";
                    sp.BaudRate = 9600;
                    sp.DataBits = 8;
                ...
                masterRtuAdown.Open(); // 打开
            }
            catch (Exception ex)
            {
                Log.Info(ex.Message);
            }
            masterRtuAleft = new ModbusRtu();
            try
            {
                masterRtuAleft.SerialPortInni(sp =>
                {
                    sp.PortName = "COM3";
                    sp.BaudRate = 9600;
                    sp.DataBits = 8;
                ...
                masterRtuAleft.Open(); // 打开
            }
            catch (Exception ex)
            {
                Log.Info(ex.Message);
            }
            masterRtuAright = new ModbusRtu();
            try
            {
                masterRtuAright.SerialPortInni(sp =>
                {
                    sp.PortName = "COM4";
                    sp.BaudRate = 9600;
                    sp.DataBits = 8;
                ...
                masterRtuAright.Open(); // 打开
            }
            catch (Exception ex)
            {
                Log.Info(ex.Message);
            }
            masterRtuBup = new ModbusRtu();
            try
            {
                masterRtuBup.SerialPortInni(sp =>
                {
                    sp.PortName = "COM5";
                    sp.BaudRate = 9600;
                    sp.DataBits = 8;
                ...
                masterRtuBup.Open(); // 打开
            }
            catch (Exception ex)
            {
                Log.Info(ex.Message);
            }
            masterRtuBdown = new ModbusRtu();
            try
            {
                masterRtuBdown.SerialPortInni(sp =>
                {
                    sp.PortName = "COM6";
                    sp.BaudRate = 9600;
                    sp.DataBits = 8;
                ...
                masterRtuBdown.Open(); // 打开
            }
            catch (Exception ex)
            {
                Log.Info(ex.Message);
            }
            masterRtuBleft = new ModbusRtu();
            try
            {
                masterRtuBleft.SerialPortInni(sp =>
                {
                    sp.PortName = "COM7";
                    sp.BaudRate = 9600;
                    sp.DataBits = 8;
                ...
                masterRtuBleft.Open(); // 打开
            }
            catch (Exception ex)
            {
                Log.Info(ex.Message);
            }
            masterRtuBright = new ModbusRtu();
            try
            {
                masterRtuBright.SerialPortInni(sp =>
                {
                    sp.PortName = "COM8";
                    sp.BaudRate = 9600;
                    sp.DataBits = 8;
            ...
            {if (!stopTemp)
                {
                    try
                    {
                       ...
                       // TimeSpan ts = stopwatch.Elapsed;
                        ...
                        //读取保持寄存器
                        var registerBuffer = masterRtuAup.Read("0",16).Content;
                        double v = (((registerBuffer[10] << 24) + (registerBuffer[11] << 16) + (registerBuffer[12] << 8) + registerBuffer[13]) * 0.0001);
                        ...
                        ControlInvoker.Invoke(this, delegate
                        {
                            ...
                            label_static_Aup.Text = registerBuffer[3] + "_" + registerBuffer[14] + "℃_Err " + registerBuffer[15];
                        });
            ...
            while (startRecv)
            {
                if (!stopTemp)
                {
                    try
                    {
                        ...
                        // masterAup = modbusFactory.CreateMaster(new TcpClient("192.168.1.200", 502));
                        //读取保持寄存器
                        byte [] registerBuffer = masterRtuAdown.Read("0", 16).Content;
                        if (registerBuffer!=null)
                        {
                            ...
                            ControlInvoker.Invoke(this, delegate
                            {
                                ...
                                label_static_Adown.Text = registerBuffer[3] + "_" + registerBuffer[14] + "℃_Err " + registerBuffer[15];
                            });
                        }
                    }
                    catch (Exception ex)
            ...
            while (startRecv)
            {
                if (!stopTemp)
                {
                    try
                    {
                        ...
                        // masterAup = modbusFactory.CreateMaster(new TcpClient("192.168.1.200", 502));
                        //读取保持寄存器
                        var registerBuffer = masterRtuAleft.Read("0", 16).Content;
                        double v = (((registerBuffer[10] << 24) + (registerBuffer[11] << 16) + (registerBuffer[12] << 8) + registerBuffer[13]) * 0.0001);
                        ...
                        ControlInvoker.Invoke(this, delegate
                        {
                            ...
                            label_static_Aleft.Text = registerBuffer[3] + "_" + registerBuffer[14] + "℃_Err " + registerBuffer[15];
                        });
            ...
            while (startRecv)
            {
                if (!stopTemp)
                {
                    try
                    {
                        ...
                        // masterAup = modbusFactory.CreateMaster(new TcpClient("192.168.1.200", 502));
                        //读取保持寄存器
                        var registerBuffer = masterRtuAright.Read("0", 16).Content;
                        double v = (((registerBuffer[10] << 24) + (registerBuffer[11] << 16) + (registerBuffer[12] << 8) + registerBuffer[13]) * 0.0001);
                        ...
                        ControlInvoker.Invoke(this, delegate
                        {
                            ...
                            label_static_Aright.Text = registerBuffer[3] + "_" + registerBuffer[14] + "℃_Err " + registerBuffer[15];
                        });
        ...
        private async void ReadBupParametersFunction()
        {
            ...
            while (startRecv)
            {
                if (!stopTemp)
                {
                    try
                    {                                      //读取保持寄存器
                        var registerBuffer = masterRtuBup.Read("0", 16).Content;
                        double v = (((registerBuffer[10] << 24) + (registerBuffer[11] << 16) + (registerBuffer[12] << 8) + registerBuffer[13]) * 0.0001);
                        ...
                        ControlInvoker.Invoke(this, delegate
                        {
                            ...
                            label_static_Bup.Text = registerBuffer[3] + "_" + registerBuffer[14] + "℃_Err " + registerBuffer[15];
                        });
                    }
                    catch (Exception ex)
                    {
        ...
        private async void ReadBdownParametersFunction()
        {
            ...
            while (startRecv)
            {
                if (!stopTemp)
                {
                    try
                    {//读取保持寄存器
                        var registerBuffer = masterRtuBdown.Read("0", 16).Content;
                        double v = (((registerBuffer[10] << 24) + (registerBuffer[11] << 16) + (registerBuffer[12] << 8) + registerBuffer[13]) * 0.0001);
                        ...
                        ControlInvoker.Invoke(this, delegate
                        {
                            ...
                            label_static_Bdown.Text = registerBuffer[3] + "_" + registerBuffer[14] + "℃_Err " + registerBuffer[15];
                        });
            ...
            while (startRecv)
            {
                if (!stopTemp)
                {
                    try
                    {
                        ...
                        // masterAup = modbusFactory.CreateMaster(new TcpClient("192.168.1.200", 502));
                        //读取保持寄存器
                        var registerBuffer = masterRtuBleft.Read("0", 16).Content;
                        double v = (((registerBuffer[10] << 24) + (registerBuffer[11] << 16) + (registerBuffer[12] << 8) + registerBuffer[13]) * 0.0001);
                        ...
                        ControlInvoker.Invoke(this, delegate
                        {
                            ...
                            label_static_Bleft.Text = registerBuffer[3] + "_" + registerBuffer[14] + "℃_Err " + registerBuffer[15];
                        });
        ...
        private async void ReadBrightParametersFunction()
        {
            ...
            while (startRecv)
            {
                if (!stopTemp)
                {
                    ...
                    {
                        //读取保持寄存器
                        var registerBuffer = masterRtuBright.Read("0", 16).Content;
                        double v = (((registerBuffer[10] << 24) + (registerBuffer[11] << 16) + (registerBuffer[12] << 8) + registerBuffer[13]) * 0.0001);
                        ...
                        ControlInvoker.Invoke(this, delegate
                        {
                            ...
                            label_static_Bright.Text = registerBuffer[3] + "_" + registerBuffer[14] + "℃_Err " + registerBuffer[15];
                        });
...

        {
            ...
            stopTemp = true;
            CloseAllCOM();
            // sentLaser(ldms80.SendBufStop,true);
            startRecv = false;
            TstartWheel = false;
            start_recv.Enabled = true;
        }
        ...
        private void ConnectTCPAllLaser()
        {
            ConnectAup();
            ConnectAdown();
            ConnectAleft();
        ...
        private void sentLaser(byte[] mode,bool rtu)
        {
        ...
        {
            stopTemp = false;
            await Task.Run(()=> ModbusRtuMasterWriteSingleRegister());
            //ConnectAllLaser();
           // sentLaser(ldms80.SendBufStart);
           // ReceiveTcpIp();
            ReceiveModbus();           
...
///B侧上点值 & B侧下点值 & B侧上下偏差值 & B侧上下状态 & B侧左点值 & B侧右点值 & B侧左右偏差值 & B侧左右状态 & B侧速度#
        ...
        {
            sentLaser(ldms80.SendBufStart,true );
            stopTemp = false;           
            timer1.Stop();
        }
    }
